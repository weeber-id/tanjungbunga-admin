...
import { Dialog } from 'components/mollecules';
  ...
  return (
    <>
      {logout && (
        <Dialog
          heading="Logout"
          message="Anda yakin ingin logout dari akun anda?"
          submitText="Logout"
          onCancel={() => setLogout(false)}
          onSubmit={() => handleLogout.mutate()}
          isLoading={handleLogout.isLoading}
        />
      )}
      <div className="h-full overflow-y-auto bg-blue-light">
        <div className="flex flex-col h-full ">
          ...
          {user?.isLoggedIn && (
            <button
              ...
              className="min-h-[5rem] w-full text-white mt-auto bg-purple-light flex justify-center items-center focus:outline-none"
            >
              Logout
            </button>
          )}
