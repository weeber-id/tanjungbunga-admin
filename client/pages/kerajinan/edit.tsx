...
import { Discussion, Handcraft, Link } from 'utils/types';
...
export const getServerSideProps: GetServerSideProps<HandcraftPageProps> = async ({
  ...
  const res = await fetch(urlApi + `/admin/handcraft?id=${id}&slug=${slug}`, {
    headers: headers,
  });
...
const EditKerajinanPage: React.FC<InferGetServerSidePropsType<typeof getServerSideProps>> = ({
  data,
}) => {
  ...
  const [state, setState] = useState<Omit<Handcraft, 'id' | 'slug' | 'active'>>({
    ...
    image: data.image,
    name: data.name,
    price: addCommas(removeNonNumeric(data.price)),
  ...
  const { data: discussions, isFetched } = useQuery<Discussion[]>('discussions', () => {
    return fetch(
      urlApi + `/admin/discussion?content_name=handcraft&content_id=${data.id}&show_answer=true`,
      {
        credentials: 'include',
      }
    )
  ...
  const handleSave = useMutation(() => {
    ...
    return fetch(urlApi + `/admin/handcraft/update?id=${data.id}`, {
      credentials: 'include',
      method: 'POST',
...
export default EditKerajinanPage;
