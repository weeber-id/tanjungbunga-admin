...
export const getServerSideProps: GetServerSideProps<KerajinanPageProps> = async ({ req }) => {
  ...
  const res = await fetch(urlApi + '/admin/handcrafts?page=1&content_per_page=7', {
    headers: headers,
  });
...
const KerajinanPage: React.FC<InferGetServerSidePropsType<typeof getServerSideProps>> = ({
  ...
  const { data: handcrafts, isSuccess } = useQuery(
    ...
    () => {
      ...
      return fetch(
        urlApi + `/admin/handcrafts?page=${currentPage}&content_per_page=7${queryParams.join('&')}`,
        {
          credentials: 'include',
        }
      )
      ...
      <div className="overflow-y-auto pb-10">
        ...
        <div className="px-12 mt-6">
          ...
          <div className="mt-4">
            ...
            {handcrafts?.data?.map(({ id, image, name, price }, i) => (
              <div
                key={id}
